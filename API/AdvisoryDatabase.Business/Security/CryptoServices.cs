    ...
    public static class CryptoServices
    {
        ...
        static CryptoServices()
        {
            ...
            var generator = new Rfc2898DeriveBytes(key,
                Encoding.ASCII.GetBytes(salt), 100);
        ...
        /// used for temporary (within session) encryption.
        /// </summary>
        ...
        public static string DecryptToken(string token)
        {
            ...
            var decryptValue = Utility.Split(DecryptText(token), new char[] { '|' });
            if (null != decryptValue && decryptValue.Length > 2)
            {
        ...
        public static Int32 DecryptTokenGetUserId(string token)
        {
            if (string.IsNullOrWhiteSpace(token)) return 0;
            var decryptValue = Utility.Split(DecryptText(token), new char[] { '|' });
            if (null == decryptValue || decryptValue.Length <= 3) return 0;
            Int32 id;
            Int32.TryParse(decryptValue[2], out id);
            return id;
        }
